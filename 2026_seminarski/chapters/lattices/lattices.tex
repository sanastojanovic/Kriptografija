...
Kako je rešetka $\latt$ diskretan skup, može se definisati minimalna dužina nenula vektora $\lambda_1(\latt) := \min \{||x|| \ | x \in \latt, x \neq 0 \} $. Element za koji važi da  mu je dužina jednaka $\lambda_1$ se naziva \textit{najkraći nenula vektor rešetke} i nalaženje ovog elementa se generalno smatra teškim \cite{smart2016cryptography}. 
...
Poseban tip rešetki su rešetke po modulu $q$ (eng. \textit{q-ary lattice}). Rešetka $\latt$ je po modulu $q$ ako važi $q \mathbb{Z}^n \subset \latt \subset \mathbb{Z}^n$ za neko celobrojno $q$. Za proizvoljnu matricu $A \in \mathbb{Z}_q^{n \times m}$ mogu se definisati dve $m$-dimenzione rešetke po modulu $q$:

\[ \Lambda_q (A) = \{ y \in \mathbb{Z}^m \mid y= A^T z \ \pmod q, z \in \mathbb{Z}^n\}, \]
\[\Lambda_q^{\bot}(A) = \{y \in \mathbb{Z}^m \mid Ay = 0 \pmod q\} \].

Rešetke po modulu $q$ igraju centralnu ulogu u konstrukciji savremenih kriptosistema zasnovanih na rešetkama i biće korišćene u nastavku rada.